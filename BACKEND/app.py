...
# List of available regions and services for the chatbot
regions = DF_RAW['region'].unique().tolist()
services = DF_RAW['resource_type'].unique().tolist()

def find_best_matches(items, query=None):
    """Helper to find best matches in a list based on query."""
    if not query:
        return []
    query = query.lower()
